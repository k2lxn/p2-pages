$(document).ready(function() {
    ...
    var test = "Bee Friendly Farming";

    $('#org_list').DataTable( {
        "ajax": {
            url: "https://massimpact.cartodb.com/api/v2/sql?q=SELECT*FROM%20mpgc_oct_import%20WHERE%20ref_organization%20ilike%20%27Bee%20Friendly%20Farming%27",
            type: "GET",
            dataSrc: "rows"
        },
        "columns": [
...
function init(){
    ...
    sql.execute("SELECT COUNT(*) FROM mpgc_database_nov where ref_organization ilike '" +test+ "' or ref_organization ilike 'Bee Buffer Project,'")
        .done(function(data) {
            // do nothing
