...
function search() {
    ...
    sql.getBounds("SELECT * FROM mpgc_database_nov where displayname ilike '" + input_org +
    ...
    sql.execute("SELECT * FROM mpgc_database_nov where displayname ilike '" + input_org + "'")
        .done(function(data) {
            // do nothing
...
function search2() {
    ...
    sql.getBounds("SELECT * FROM mpgc_database_nov WHERE ((city = '" +input_city+ "') AND (state_province = '" + input_state + "'))").done(function(bounds2) {
        map.fitBounds(bounds2)
        // restricts the map view to the given bounds
        //map.setMaxBounds(bounds2) (Enabling this will lock in search area to bounds
    });

    sql.execute("SELECT * FROM mpgc_database_nov where city = '" + input_city +
            "'")
        .done(function(data) {
            // do nothing
            for (var i = 0; i < data.total_rows; i++) {
...
function search3() {
    ...
    var sql = new cartodb.SQL({ user: 'massimpact' });
    sql.getBounds("SELECT * FROM mpgc_database_nov where displayname ilike '" + input_username +
    ...
    sql.execute("SELECT * FROM mpgc_database_nov where username like '" + input_username +
            "'")
        .done(function(data) {
            // do nothing
            for (var i = 0; i < data.total_rows; i++) {
...
$.getJSON("https://massimpact.cartodb.com/api/v2/sql?format=GeoJSON&q=SELECT * FROM mpgc_database_nov", function(data) {
