...
function search() {
    ...
    sql.getBounds("SELECT * FROM mpgc_userdata where displayname ilike'" + input_org +
    ...
    sql.execute("SELECT * FROM mpgc_userdata where displayname ilike '" + input_org + "'")
        .done(function(data) {
            // do nothing
        ...
        .done(function(data) {
            ...
            for (var i = 0; i < data.total_rows; i++) {
                $('#results').append("<div class='city_result'><h3>" +
                    ...
                    data.rows[i].city +
                    " "+
                    data.rows[i].organization_description +
                    "</h5></span>"+
                    "</div></h5>");
            }
        })
...
function search3() {
    ...
    var sql = new cartodb.SQL({ user: 'massimpact' });
    sql.getBounds("SELECT * FROM mpgc_submissions where street ilike '" + input_username +
